...
def getModel(config, embedding_matrix):
    ...
    embedding_layer = Embedding(input_dim=config["vocab_length"],
                                ...
                                input_shape=(config["input_length"],),
                                trainable=False,
    ...
    model = Sequential([
        embedding_layer,
        # Can't export the model with Bidirectional at the moment
        # Bidirectional(LSTM(config["vector_size"], dropout=0.3, return_sequences=True), name='layer_bidirectional'),
        LSTM(config["vector_size"], dropout=0.3, return_sequences=True, name='layer_ltsm'),
        Conv1D(config["vector_size"], 5, activation='relu', name='layer_conv1d'),
        GlobalMaxPool1D(name='later_globalmaxpool1d'),
        Dense(16, activation='relu',name='layer_dense_1'),
        Dense(1, activation='sigmoid',name='layer_dense_2'),
    ],
